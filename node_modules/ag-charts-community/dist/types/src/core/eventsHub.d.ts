import type { AxisID, BaseStyleTypeMap, ChartAxisDirection, ChartUpdateType, DeepReadonly, DefinedZoomState, Scale, ZoomMinMax, ZoomMinMaxDirection, ZoomState } from 'ag-charts-core';
...
export interface EventsHubMap {
    ...
    'axis:change': null;
    'axis-dom-proxy:cursor': {
        cursor: BaseStyleTypeMap['cursor'] | undefined;
    };
    'axis-dom-proxy:toggle-dragging-cursor': {
...
export type ZoomEventSourceDetail = `contextmenu-pan-to-cursor` | `contextmenu-reset` | `contextmenu-zoom-to-cursor` | `dataSource` | `internal-applyOptions` | `internal-autoScaling` | `internal-panToBBox` | `internal-prepareResizedChart` | `internal-restoreMemento` | `internal-requiredWidth` | `internal-setAxes` | `internal-updateSyncZoom` | `keyboard(${-1 | 0 | 1})` | `keyboard-page(${'home' | 'end' | -1 | 1})` | `navigatorDOM` | `navigator` | `onDataChange-reset` | `unspecified` | `zoom-axis-dblclick` | `zoom-axis-drag` | `zoom-axis-wheel` | `zoom-button-pan-end` | `zoom-button-pan-left` | `zoom-button-pan-right` | `zoom-button-pan-start` | `zoom-button-reset` | `zoom-button-zoom-in` | `zoom-button-zoom-out` | `zoom-range-button-${number}` | `zoom-seriesarea-dblclick` | `zoom-seriesarea-panner` | `zoom-seriesarea-selector` | `zoom-seriesarea-twofingers` | `zoom-seriesarea-wheel` | `scrollbar`;
