...
import type { CartesianMarkerLikeContext, CartesianSeriesNodeDataContext, CartesianSeriesNodeDatum } from './cartesianSeriesTypes';
...
export declare function prepareLinePathStrokeAnimationFns(status: NodeUpdateState, spans: SpanAnimation, visibleToggleMode: 'fade' | 'none', targetOpacity?: number): {
    ...
    pathProperties: FromToFns<unknown, Path<unknown>, any>;
};
export declare function prepareLinePathPropertyAnimation(status: NodeUpdateState, visibleToggleMode: 'fade' | 'none', targetOpacity?: number): FromToFns<unknown, Path<unknown>, any>;
export declare function prepareLinePathAnimation(newData: LineSeriesNodeDataContext, oldData: LineSeriesNodeDataContext, diff: ProcessedOutputDiff | undefined, targetOpacity?: number): {
    status: NodeUpdateState;
    stroke: {
        status: NodeUpdateState;
        path: {
            addPhaseFn: (ratio: number, path: Path<unknown>) => void;
            updatePhaseFn: (ratio: number, path: Path<unknown>) => void;
            removePhaseFn: (ratio: number, path: Path<unknown>) => void;
        };
        pathProperties: FromToFns<unknown, Path<unknown>, any>;
    };
    hasMotion: boolean;
