  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-5">
        <Input placeholder="Other country — type and press Enter"
          value={customCountry} onChange={e => setCustomCountry(e.target.value)}
          onKeyDown={e => { if (e.key === "Enter" && customCountry.trim()) setConfig({ countryCode: "CUSTOM" }) }}
          className="text-sm" />
        <Button variant="outline" onClick={() => customCountry.trim() && setConfig({ countryCode: "CUSTOM" })}>Use</Button>
      </div>
      {selected && (
